...
- GDGuide style as in existing files: tabs for indentation; `snake_case` files and methods; private members prefixed `_`; tuning values exposed as `@export` with `##` doc comments. Each script `extends` a Godot node type (no `class_name` declarations in use).
